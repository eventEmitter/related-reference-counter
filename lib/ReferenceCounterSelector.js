        ...
        var ClassConstructor = new Class({
           ...
           , name: 'referenceCount'
            ...
            , init: function init(options) {
                ...
                // filters anyone?
                if (options.parameters.length > 1) {
                    this.filters = options.parameters[1];
                }
            }
            ...
            , _joinEntites: function(queryBuilder, path) {
                if (path.length) this._joinEntites(queryBuilder.leftJoin(path[0], true), path.slice(1));
             ...
             */
            , prepare: function(queryBuidler) { log(this);
                // join tables
                this._joinEntites(queryBuidler, this.path.split('.'));

                // filters?
                if (this.filters) queryBuidler.filter(this.filters);
            }
        });
