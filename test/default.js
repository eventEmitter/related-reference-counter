	...
	describe('Travis', function(){
		...
		it('should be able to drop & create the testing schema ('+sqlStatments.length+' raw SQL queries)', function(done) {
			related.getDatabase('related_refcount_test').getConnection(function(err, connection) {
				if (err) done(err);
				else {
	...
	describe('The Counter Extension', function() {
		var oldDate;

		it('should not crash when instatiated', function() {
			db = related.related_refcount_test;
			extension = new ReferenceCounter();
		});
	...
	describe('Querying', function() {
		it('Using the referenceCount selector', function(done) {
			db.event(['*', Related.select('count').referenceCount('venue.id')])
			.order('count', true).limit(10).find(done);
		});
	});
